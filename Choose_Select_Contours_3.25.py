...
order_num = 0 
read_path =  'H:\\TrafficSignData\\TrafficSignData_Norm\\4 上坡\\' 
save_path =  'H:\\OUT\\PIC\\4 上坡\\' 
txt_path  =  'H:\\OUT\\Label\\4 上坡\\labels.txt'
...
for order_num in xrange(start_picture,Len+1):
    ...
    for i in contours:
        ...
        if (w*h <= min_pic):
            continue
        cv2.rectangle(show,(x,y),(x+w,y+h),(0,255,0),1)
    cv2.destroyAllWindows()
    cv2.imshow(str(order_num) + '.jpg',show)
    k = cv2.waitKey(0)&0xFF
    while k != ord('Y'):
